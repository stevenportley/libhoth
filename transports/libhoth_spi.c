...
int libhoth_tpm_spi_probe(struct libhoth_device* dev) {
  ...
  xfer[0].tx_nbits = sizeof(tx_buf);

  xfer[1].rx_buf = (uint64_t)rx_buf;
  xfer[1].len = sizeof(rx_buf);
  xfer[1].rx_nbits = sizeof(rx_buf);
  ...
  uint32_t did_vid = (uint32_t)rx_buf[1] << 24 | (uint32_t)rx_buf[2] << 16 |
                     (uint32_t)rx_buf[3] << 8 | (uint32_t)rx_buf[4] << 0;
